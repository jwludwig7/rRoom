...
import re
EMAIL_REGEX = re.compile(r'^[a-zA-Z0-9.+_-]+@[a-zA-Z0-9._-]+\.[a-zA-Z]+$')
from flask_app.models import review_model
...
# this is to be able to see the email that was just entered
    ...
    def get_by_id(cls,data):
        query = "SELECT * FROM users LEFT JOIN reviews ON users.id = reviews.user_id LEFT JOIN restrooms on restrooms.id = reviews.restroom_id WHERE users.id = %(id)s;"
        results = connectToMySQL(DATABASE).query_db(query,data)
        if len(results) > 0:
            user = cls(results[0])
            list_of_reviews = []
            for row_in_db in results:
                ...
                this_review = review_model.Review(review_data)
                list_of_reviews.append(this_review)
            user.reviwes = list_of_reviews
            return user
        return False
